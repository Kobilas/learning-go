...
type player interface {
	getMove(boardState *[3][3]int)
}

func (p human) getMove(boardState *[3][3]int) (bool, error) {
	var ans string
	var idx int
	for {
		fmt.Print(p.name, "where would you like to move? (enter an index) ")
		fmt.Scan(&ans)
		idx, err = strconv.Atoi(ans)
		if err != nil {
			return false, err
		} else {
			if !checkAvailability(boardState, idx) {
				return false, &moveError{"Space Unavailable Error", p.name, idx}
			} else {
